...
import { Trophy, CalendarCheck, Users, UsersRound, LogOut, Swords } from 'lucide-react';
...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<'matches' | 'leaderboard' | 'challenges'>('matches');
  
  const [inviteId, setInviteId] = useState<string | null>(null);
  const [inviteChallengeName, setInviteChallengeName] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    if (invite && supabase) {
      ...
      supabase.from('challenges').select('title, match_home_team, match_away_team').eq('id', invite).single()
        .then(({ data }) => {
          if (data) {
            setInviteChallengeName(`${data.title} (${data.match_home_team} vs ${data.match_away_team})`);
          }
      ...
      return (
        <div className="min-h-screen bg-gray-50 flex flex-col justify-center items-center p-6 text-center">
          <div className="bg-white p-8 rounded-3xl shadow-xl max-w-sm w-full border border-gray-100">
            ...
            <h1 className="text-2xl font-bold text-gray-800 mb-2">Tu as été invité !</h1>
  ...
  const getWhatsappLink = () => {
    const inviteText = encodeURIComponent("Rejoins mon groupe de paris sportifs ! Prépare tes pronos pour les prochains matchs. ⚽️🏆");
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans">
      <header className="bg-emerald-700 text-white shadow-md pt-10 pb-4 px-6 md:px-12 flex justify-between items-center rounded-b-3xl">
        <div className="flex items-center gap-3">
          <img src={logoImage} alt="Mirfoot Logo" className="w-12 h-12 object-cover rounded-xl shadow-sm bg-white" />
          <div>
            <h1 className="text-2xl font-bold tracking-tight mb-0 text-emerald-300">Mirfoot</h1>
            <div className="font-handwriting text-xl text-white transform -rotate-2 -mt-1 mb-1">Halloufa 2026</div>
            <p className="text-emerald-100 text-sm font-medium">Ligue des Potes</p>
          </div>
        </div>
        <div className="flex gap-2">
          ...
          >
            ...
            <span className="hidden sm:block text-sm font-semibold">Inviter</span>
          </a>
          <button 
