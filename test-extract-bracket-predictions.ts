...
async function run() {
  ...
  const { data: challenges, error: err1 } = await supabase.from("challenges").select("id, competition_id").eq("competition_id", 9999);
  if (err1) {
    console.error("Error fetching World Cup challenges:", err1);
    return;
  }
  
  console.log(`Found ${challenges.length} 9999 challenges.`);
  for (const c of challenges) {
    ...
    bets.forEach(b => {
      ...
      console.log(`  predictions:`, JSON.stringify(b.predictions));
    });
  }
}
